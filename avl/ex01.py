...
class AVLTree(BST):
    ...
    def print_as_normal(root, deep = 0):
        if (root == None):
            return
        AVLTree.print_as_normal(root.right, deep + 1)
        print("    " * deep, end = "")
        print(root.data)
        AVLTree.print_as_normal(root.left, deep + 1)
...
for i in inp:
    if (ad.get(i) == None):
        ad.update({i:inp.count(i)})

sorted_ad = sorted(ad.items(), key=lambda x:x[1])
sorted_ad= dict(sorted_ad)
print(sorted_ad)
keys = sorted_ad.keys()
for key in keys:
...
AVLTree.print_as_normal(avl1.root)
# avl1.add(1)
# print(avl1)
# avl1.add(2)
# print(avl1)
