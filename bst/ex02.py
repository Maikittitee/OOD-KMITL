class Node:
	...
	def	get_height(node: "Node", deep = -1):
		if (node == None):
			return deep
		return (max(Node.get_height(node.right, deep + 1), Node.get_height(node.left, deep + 1)))
...
class BST:
	...
	def get_height(self):
		return (Node.get_height(self.root))
...
for i in inp:
	t.insert(i)
print(f"Height of this tree is : {t.get_height()}")
